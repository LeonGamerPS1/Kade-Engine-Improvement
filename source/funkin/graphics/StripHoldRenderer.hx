...
class SustainTrail extends FlxSprite
{
	...
	var fullSustainLength:Float = 0;
	var noteDirection:Int = 0;
	 ...
	 * @param fileName
	 */
	public function new(noteDirection:Int, sustainLength:Float, noteStyle:String = "normal")
	{
		super(0, 0);
	 ...
	 */
	public function setupHoldNoteGraphic(style:String = "default"):Void
	{
		loadGraphic(Paths.image('holdNotes/$style', 'shared'));
	...
	override function update(elapsed)
	{
		...
		if (previousScrollSpeed != (speed))
		{
			triggerRedraw();
			previousScrollSpeed = speed;
		}
	}
	...
	{
		graphicHeight = sustainHeight(sustainLength, speed);
		updateClipping();
		updateHitbox();
	}
	...
	public function updateClipping(songTime:Float = 0):Void
	{
		...
		var clipHeight:Float = FlxMath.bound(sustainHeight(sustainLength - (songTime - time), speed), 0, graphicHeight);
		if (clipHeight <= 0.1)
		{
			visible = false;
			return;
