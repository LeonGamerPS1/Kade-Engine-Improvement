...
{
	var noteTypeList:Array<String> = ['', 'Alt Animation', 'Hey!'];
	...
	**/
	...
	function addSectionUI():Void
	{
		...
		var pasteButton:FlxButton = new FlxButton(10, 180, "Paste Section", function()
		{
			for (note in _song.notes[sectionToCopy].sectionNotes)
			{
				...
				{
					copiedNote = [
						note[0] + Conductor.stepCrochet * (_song.notes[sectionToCopy].lengthInSteps * (curSection - sectionToCopy)),
						note[1],
						note[2],
						note[3],
						note[4]
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		note.y = (GRID_SIZE * (isNextSection ? 16 : 0))
			+
	...
	function setupSusNote(note:Note):FlxSprite
	{
		var spr:FlxSprite = new FlxSprite(note.x + (GRID_SIZE * 0.4),
			note.y + GRID_SIZE).makeGraphic(8,
				Math.floor(FlxMath.remapToRange(note.sustainLength, 0, Conductor.stepCrochet * 16, 0, gridBG.height / gridMult)));
		return spr;
	}
	...
	function selectNote(note:Note):Void
	{
		...
		for (i in _song.notes[curSection].sectionNotes)
		{
			if (i[0] == note.strumTime && Std.int(i[1]) % 4 == note.noteData)
			{
